  ...
  private async _handleRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    if (loginMatch && method === 'POST') {
      ...
      if (name.endsWith('-api')) {
        json(res, 400, { status: 'error', provider: name, message: `${name} uses API keys, not browser login. Set your key via: cortex config apiKeys.${name} <key>` });
        return;
      }
    ...
    if (url === '/v1/chat/completions' && method === 'POST') {
      ...
      const { model, messages, stream = false, temperature, max_tokens, newConversation = false } = req_data;
      if (!model || !messages) {
