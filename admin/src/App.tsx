...
const sections: Array<{
  ...
  { id: 'accounts', label: 'Account Router', icon: UsersRound, permission: 'providers:manage', caption: 'Multi-account routing per provider' },
  { id: 'browsers', label: 'Browser Sessions', icon: LayoutGrid, permission: 'providers:manage', caption: 'Live view of every Chromium + VNC' },
...
function App() {
  ...
  // Auto-close mobile nav when section changes; lock body scroll while open.
  ...
  if (admin) {
    const activeSection = visibleSections.find(s => s.id === active)
    return (
      <div className="ui-app-shell">
        {/* Mobile top bar (lg:hidden) */}
        <header className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-slate-200 bg-white/95 px-4 backdrop-blur-md shadow-sm lg:hidden">
          <button
            type="button"
          ...
          <img src="/logo.svg" alt="Cortex Admin" className="h-7 w-auto" />
          <div className="ml-auto truncate text-sm font-semibold text-slate-700">
            {activeSection?.label ?? 'Cortex'}
          </div>
        </header>

        {/* Mobile drawer backdrop + panel (lg:hidden) */}
        <div
        ...
        <aside
          ...
          className={`fixed inset-y-0 left-0 z-50 flex w-[min(280px,86vw)] flex-col bg-white shadow-2xl transition-transform duration-300 ease-out lg:hidden ${
        ...
        >
          <div className="flex items-center justify-between border-b border-slate-200 p-4">
            <div className="flex items-center gap-2">
              ...
              <span className="rounded-full border border-blue-200 bg-blue-50 px-2 py-0.5 text-[10px] font-semibold text-blue-700">Control</span>
            </div>
            <button
              type="button"
          ...
          <nav className="ui-scroll flex-1 space-y-1 overflow-y-auto p-3">
            {visibleSections.map(section => {
              ...
              return (
                <button
                  ...
                  onClick={() => { setActive(section.id); setMobileNavOpen(false) }}
                >
                  <div className="flex items-center gap-2.5">
                    <section.icon size={16} />
                    <span className="text-sm font-semibold">{section.label}</span>
                  </div>
                  <p className="mt-0.5 text-xs text-slate-500">{section.caption}</p>
                </button>
              )
            })}
          </nav>
          <div className="border-t border-slate-200 p-3">
            ...
            <p className="mt-0.5 truncate text-sm font-semibold text-slate-900">{admin.username}</p>
        ...
        <div className="grid min-h-screen grid-cols-1 gap-4 p-3 sm:p-4 lg:grid-cols-[280px_minmax(0,1fr)] lg:items-start lg:pl-0">
          ...
          <div className="space-y-4">
            ...
            <main className="pb-6">
              ...
              {active === 'accounts' ? <AccountsPage /> : null}
              {active === 'browsers' ? <BrowsersPage /> : null}
