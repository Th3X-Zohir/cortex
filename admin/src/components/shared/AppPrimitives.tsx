  ...
  return (
    <>
      <div className="mb-7 flex flex-col justify-between gap-4 md:flex-row md:items-start">
        <div>
          <p className="text-[11px] uppercase tracking-[0.24em] text-white/50">Operations Workspace</p>
          <h1 className="mt-2 bg-gradient-to-r from-white via-white to-white/78 bg-clip-text text-3xl font-extrabold text-transparent">{title}</h1>
...
export function Panel({ title, description, children, className = '' }: { title: string; description?: string; children: ReactNode; className?: string }) {
  return (
    <section className={`rounded-2xl border border-white/12 bg-white/[0.035] p-5 shadow-[0_16px_30px_rgba(4,11,24,0.36)] backdrop-blur-xl ${className}`}>
      <div className="mb-4">
        <h2 className="text-lg font-semibold text-white">{title}</h2>
        {description && <p className="mt-1 text-sm text-white/62">{description}</p>}
      </div>
      {children}
    </section>
  )
...
export function Metric({ label, value, helper, icon: Icon, tone = 'neutral' }: { label: string; value: string; helper: string; icon: typeof Activity; tone?: 'neutral' | 'good' | 'warn' | 'bad' }) {
  ...
  return (
    <div className="rounded-2xl border border-white/12 bg-white/[0.035] p-5 shadow-[0_14px_26px_rgba(4,11,24,0.34)] backdrop-blur-xl">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="label text-white/50">{label}</p>
          <p className="mt-3 text-3xl font-bold text-white">{value}</p>
          <p className="mt-2 text-sm text-white/56">{helper}</p>
        </div>
...
export function NavButton({ active, icon: Icon, label, onClick, collapsed = false }: { active: boolean; icon: typeof Activity; label: string; onClick: () => void; collapsed?: boolean }) {
  return (
    <button
      className={`group flex w-full items-center rounded-lg border py-2.5 text-left text-sm font-semibold transition-all ${collapsed ? 'justify-center px-2' : 'gap-3 px-3'} ${active ? 'border-primary/30 bg-primary/14 text-primary-light shadow-[0_8px_20px_rgba(37,99,235,0.24)]' : 'border-transparent text-white/62 hover:border-white/12 hover:bg-white/[0.06] hover:text-white'}`}
      onClick={onClick}
      title={collapsed ? label : undefined}
      aria-label={label}
    >
      <div className={`flex h-8 w-8 items-center justify-center rounded-md border ${active ? 'border-primary/32 bg-primary/16' : 'border-white/12 bg-white/[0.03]'} transition-all`}>
        <Icon size={16} />
      </div>
...
export function RefreshButton({ onClick, loading }: { onClick: () => void; loading: boolean }) {
  return (
    <button className="btn-ghost border-white/12 bg-white/[0.03]" onClick={onClick} disabled={loading}>
...
export function EmptyState({ icon: Icon, message }: { icon: typeof Activity; message: string }) {
  return (
    <div className="flex min-h-40 flex-col items-center justify-center gap-3 text-center text-white/48">
      <Icon size={28} />
      <p>{message}</p>
    </div>
  )
...
export function FullScreenLoader() {
  return (
    <div className="flex min-h-screen items-center justify-center">
      <div className="rounded-2xl border border-white/12 bg-background-secondary/88 px-8 py-6 text-center backdrop-blur-xl">
        <Loader2 className="mx-auto animate-spin text-primary" size={32} />
        <p className="mt-3 text-sm text-white/68">Loading command center...</p>
      </div>
    </div>
  )
}
...
export function FullWidthLoading() {
  return (
    <div className="flex min-h-64 items-center justify-center rounded-2xl border border-white/12 bg-white/[0.035] shadow-[0_16px_32px_rgba(4,11,24,0.36)] backdrop-blur-xl">
